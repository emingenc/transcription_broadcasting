/**
 * HTML Templates for Mentra Cast webviews.
...
export function mainWebview(params: WebviewParams): string {
  const { userId, mode, broadcaster, isLive, listenerCount } = params;
...
<body>
  ...
  <div id="broadcast-mode" class="content ${mode !== "broadcast" ? "hidden" : ""}">
    <div class="card">
      <div class="status-bar ${isLive ? "status-live" : "status-idle"}" id="my-status">
        ${isLive ? "● LIVE" : "● OFFLINE"}
      </div>
      <div style="margin-top: 16px;">
        <button class="btn ${isLive ? "btn-danger" : "btn-primary"}" onclick="toggleBroadcast()">
          ${isLive ? "STOP BROADCAST" : "GO LIVE"}
        </button>
      </div>
    </div>

    <div class="card" ${!isLive ? 'style="opacity:0.5;pointer-events:none"' : ""}>
      <div class="label">Listeners</div>
      <div class="listener-count" id="listener-count">${listenerCount}</div>
    ...
    <div class="card" ${!isLive ? 'style="opacity:0.5;pointer-events:none"' : ""}>
      ...
      <button class="btn btn-primary" onclick="sendMessage()">Send to Glasses</button>
    </div>
  </div>
  ...
  <script>
    ...
    // Restore broadcaster from localStorage
    ...
    function switchMode(mode) {
      ...
      if (mode === 'listen' && broadcaster) url += '&broadcaster=' + encodeURIComponent(broadcaster);
      location.href = url;
    }

    async function toggleBroadcast() {
      await fetch(isLive ? '/api/broadcast/stop' : '/api/broadcast/start', { method: 'POST' });
      location.reload();
    }
    ...
    function joinBroadcast() {
      ...
      }).then(() => location.href = '/webview?mode=listen&broadcaster=' + encodeURIComponent(email));
    }
    ...
    function leaveBroadcast() {
      ...
      }).then(() => location.href = '/webview?mode=listen');
    }
    ...
    if (currentMode === 'listen' && broadcaster) {
      ...
      setInterval(async () => {
        try {
          ...
          statusEl.textContent = data.live ? '● LIVE' : '● OFFLINE';
          statusEl.style.background = data.live ? '#ff3b30' : '#e5e5e5';
          statusEl.style.color = data.live ? 'white' : '#666';
          
          if (data.messages && data.messages.length !== lastCount) {
            const chat = document.getElementById('chat');
            chat.innerHTML = data.messages.length === 0 
              ? '<div class="empty">Waiting for messages...</div>'
              : data.messages.map(m => '<div class="bubble"><div class="text">' + m.text + '</div><div class="time">' + new Date(m.time).toLocaleTimeString() + '</div></div>').join('');
